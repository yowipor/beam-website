...
class CraftParser:
    ...
    def extract_full_entry_text(self, entry: str) -> str:
        return normalize_text(entry)

    def clean_item_display(self, entry: str) -> str:
        return re.sub(r"\s+", " ", entry.strip())

    def detect_paint_from_entry(self, label: str, full_entry: str):
        slot_priority_labels = {
            "trim a", "trim b", "trim c",
            "engine blocks", "engine block",
            "engine struts", "engine strut",
            "air filters", "air filter",
            "engine exhaust", "engine exhausts",
            "fuel tanks",
            "arch covers",
            "aerials",
            "extras",
        ...
        if label in slot_priority_labels:
            return None
        ...
        if any(keyword in full_entry for keyword in paint_keywords):
            return "paint"

        if label in {
            "primary", "secondary", "pearlescent", "dashboard", "interior",
            "wheel color", "wheel colouring", "wheel colour", "wheel paint"
        }:
            return "paint"

        return None

    def map_label(self, label: str, full_entry: str = ""):
        if label in self.alias_to_craft:
            return self.alias_to_craft[label]

        paint_match = self.detect_paint_from_entry(label, full_entry)
        if paint_match:
            return paint_match
    ...
    def parse(self, text: str):
        ...
        for entry in self.split_entries(text):
            clean_entry = self.clean_item_display(entry)
            label = self.extract_label(entry)
            full_entry = self.extract_full_entry_text(entry)
            craft = self.map_label(label, full_entry)

            if craft:
                counts[craft] += 1
                value = self.recipes.get(craft, {}).get("value")
                if isinstance(value, int) and value > 0:
                    itemized.append(f"{clean_entry} → {craft} (${value})")
                else:
                    itemized.append(f"{clean_entry} → {craft}")
            else:
                itemized.append(f"{clean_entry} → unmapped")
  ...
  <style>
    ...
    *{box-sizing:border-box}
    body{
      ...
      color:var(--text);
      background:
        linear-gradient(rgba(4,7,12,.55), rgba(4,7,12,.78)),
        url('/static/bg.jpg') center center / cover fixed no-repeat,
        var(--bg);
    }
    .page{max-width:1200px;margin:0 auto;padding:28px 18px 36px;}
    .hero{
    ...
    .brand{display:flex;align-items:end;gap:14px;flex-wrap:wrap;margin-bottom:10px;}
    .brand h1{margin:0;font-size:clamp(2rem,4vw,3.6rem);letter-spacing:.08em;line-height:1;text-transform:uppercase;}
    .tag{
      display:inline-flex;align-items:center;padding:7px 12px;border-radius:999px;font-size:.85rem;color:#fff;
      background:linear-gradient(135deg, var(--accent), var(--accent-2));
      box-shadow:0 8px 20px rgba(124,92,255,.28);
    }
    .sub{color:var(--muted);margin:0 0 18px;max-width:760px;font-size:1rem;line-height:1.5;}
    .input-wrap{display:flex;flex-direction:column;gap:12px;}
    textarea{
      width:100%;min-height:190px;resize:vertical;border-radius:20px;border:1px solid rgba(255,255,255,.14);
      background:rgba(6,10,16,.72);color:var(--text);padding:18px 18px;font:inherit;line-height:1.5;outline:none;
    }
    textarea:focus{border-color:rgba(255,79,135,.65);box-shadow:0 0 0 4px rgba(255,79,135,.14);}
    .actions{display:flex;gap:12px;flex-wrap:wrap;align-items:center;}
    button{
      appearance:none;border:none;border-radius:16px;padding:14px 18px;font:inherit;font-weight:700;color:white;
    ...
    .card{
      grid-column:span 6;border:1px solid var(--line);
      background:linear-gradient(180deg, rgba(17,21,31,.84), rgba(11,14,21,.9));
      backdrop-filter: blur(10px);border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden;
    }
    .card.full{grid-column:1/-1}
    .card-header{
      padding:16px 18px 12px;display:flex;justify-content:space-between;align-items:center;gap:12px;
      border-bottom:1px solid rgba(255,255,255,.06);
    }
    .card-title{margin:0;font-size:1.02rem;}
    .count-badge{
      min-width:34px;padding:6px 10px;border-radius:999px;text-align:center;color:#fff;font-size:.84rem;
      background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.08);
    }
    .card-body{padding:16px 18px 18px;}
    ul.clean{list-style:none;margin:0;padding:0;display:grid;gap:10px;}
    ul.clean li{
      display:flex;justify-content:space-between;gap:14px;padding:11px 12px;border-radius:14px;
      background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.05);align-items:flex-start;
    }
    ul.clean li .label{color:var(--text);font-weight:600;}
    ul.clean li .value{color:#fff;font-weight:800;white-space:nowrap;}
    .item-list li{justify-content:flex-start;color:#eef2f8;font-weight:500;word-break:break-word;}
    .pill{
      display:inline-flex;align-items:center;justify-content:center;gap:8px;min-height:42px;padding:10px 14px;
      border-radius:14px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);font-weight:800;
    }
    .cost{font-size:2rem;font-weight:900;letter-spacing:.02em;}
    .empty{color:var(--muted);margin:0;}
    .footer{margin-top:18px;color:rgba(255,255,255,.72);font-size:.92rem;text-align:center;}
    ...
    <section class="hero">
      ...
      <form method="post" class="input-wrap">
        ...
        <div class="actions">
          ...
          <span class="hint">Matches the current Discord bot logic.</span>
        </div>
      </form>
    </section>
